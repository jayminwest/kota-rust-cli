...
use super::{
    ApprovalMode, ApprovalRequest, ApprovalSystem, PolicyEngine, SandboxProfile, SandboxedCommand,
};
...
impl SecureExecutor {
    ...
    async fn execute_internal_command(&self, command_str: &str) -> Result<String> {
        ...
        match self
            .command_registry
            .execute(command, arg, &mut context, &mut model_config)?
        {
            Some(CommandResult {
                success: true,
                output,
