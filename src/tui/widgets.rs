...
use super::app::App;
use super::types::{CommandStatus, FocusedPane, InputMode, MessageContent};
...
pub fn process_markdown_for_display(content: &str) -> String {
    ...
    for (line_idx, line) in lines.iter().enumerate() {
        ...
        let truncated_line = if line.len() > 60 {
            // Reduced from 80 to 60 for better fit
            format!("{}...", &line[..57])
        } else {
            line.to_string()
        };
        ...
        if truncated_line.starts_with("```") {
            ...
            if in_code_block {
                processed.push_str(&format!(
                    "[CODE] {}",
                    truncated_line.trim_start_matches("```")
                ));
            } else {
...
pub fn create_chat_view(app: &App) -> Paragraph {
    ...
    if app.messages.is_empty() {
        lines.push(Line::from(vec![Span::styled(
            "No messages yet. Try typing 'i' and sending a message.",
            Style::default()
                .fg(Color::DarkGray)
...
pub fn create_terminal_view(app: &App) -> Paragraph {
    ...
    if start_idx > 0 {
        lines.push(Line::from(vec![Span::styled(
            format!("... ({} earlier lines hidden)", start_idx),
            Style::default()
                .fg(Color::DarkGray)
                .add_modifier(Modifier::ITALIC),
        )]));
    ...
    if !app.suggested_commands.is_empty() {
        ...
        for (i, cmd) in app.suggested_commands.iter().enumerate() {
            ...
            lines.push(Line::from(vec![Span::styled(
                format!("{}{}[{}] {}", prefix, i + 1, status_indicator, cmd.command),
                style,
            )]));
        }
        ...
        lines.push(Line::from(""));
        lines.push(Line::from(vec![Span::styled(
            "Commands: x=execute n=next p=prev c=clear",
    ...
    let title = if !app.suggested_commands.is_empty() {
        format!(
            " KOTA Terminal ({} commands) ",
            app.suggested_commands.len()
        )
    } else {
...
pub fn create_context_view(app: &App) -> Paragraph {
    ...
    let context_preview = if app.context_view.len() > 250 {
        format!(
            "{}...\n[{} more chars]",
            &app.context_view[..250],
            app.context_view.len() - 250
        )
    } else {
...
pub fn create_file_browser(app: &App) -> Table {
    ...
    let rows: Vec<Row> = app
        ...
        .map(|(i, item)| {
            ...
            Row::new(vec![
                Cell::from(name),
                Cell::from(if item.is_dir {
                    "-".to_string()
                } else {
                    FileBrowser::format_size(item.size)
                }),
...
pub fn create_status_bar(app: &App) -> Paragraph {
    ...
    Paragraph::new(status).style(Style::default().bg(Color::Black).fg(Color::White))
}
...
pub fn create_small_terminal_warning() -> Paragraph<'static> {
    let warning_text = vec![
        ...
        Line::from(""),
        Line::from(vec![Span::raw("Minimum size required: 80x24")]),
